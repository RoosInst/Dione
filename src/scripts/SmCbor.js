...
var smCbor = (function() {
  ...
  var getArray = function() {
    let rtn = [];
    while(!atArrayEnd()) {
      rtn.push(getNext()); //QUES: Should this be a recursive expand? any special features in the returned object?
    }
    return rtn;
  };
  ...
  var getOmap = function() {
    ...
    let val = [];
    let attribs = {
      tag: getNext(),     //first thing in an omap is the name, so fetch that
      value: "adminInfo", //QUES: not sure what goes in here
      //err: ""           //QUES: not sure if this field is required unless there is an error
    };
    val.push(attribs);
    while(!atArrayEnd()) {
      val.push(getNext()); //QUES: recursive expansion?
    }
