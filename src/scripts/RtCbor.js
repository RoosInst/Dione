...
var rtCbor = (function() {
  ...
  var getNext = function() {
    if(peekNext()===null) return null; //nothing left to get
    ...
    switch(theType) {
      ...
      default:
        console.log('CBOR Error. Unrecognized type: ' + theType);
  ...
  */
  ...
  /**Rewinds the read sequence to the first item in the buffers arrays*/
  var rewind = function() {
    whichBuffer = 0; //which Buffer in buffers
    curPos = 0; //index into the current buffer
  };
  ...
  var decodeAll = function () {
    var ret = [];
    while(peekNext()!==null) { //while there is stuff left to get
      let d = getNext();
      if(d) ret.push(d);
    }
