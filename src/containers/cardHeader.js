...
import { updateWhiteboard, updateWhiteboardTabs, updatePaneSize } from '../actions';
...
export class CardHeader extends Component {
    handleClose(model) { //delete app, clicking on close 'X' button
        ...
        */
        ...
        // const {connectionDetails, subscriptions, updatePaneSize} = this.props;
        ...
        updatePaneSize(model, 0, 0, 'remove');

        return null;
    }

    handleMinimize(model) {
        const {whiteboard, updateWhiteboard, updateWhiteboardTabs} = this.props;
        updateWhiteboardTabs(model, whiteboard[model])
        let forest = $.extend({}, this.props.whiteboard);
        delete forest[model];
...
export default connect(mapStateToProps, { updateWhiteboard, updateWhiteboardTabs, updatePaneSize })(CardHeader);
